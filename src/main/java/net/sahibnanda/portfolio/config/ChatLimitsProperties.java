 ...
 * grows. Product configuration, not secret -- set directly in
 * application.yml rather than sourced from an environment variable.
 *
 ...
 * @param maxHistoryMessages maximum number of the most recent messages sent
 *        to the LLM per turn, regardless of how many are actually stored
 * @param maxHistoryChars maximum combined character budget of the messages
 *        sent to the LLM per turn, applied on top of
 *        {@code maxHistoryMessages}
 * @param maxMessagesPerChat maximum number of messages a single chat may
 *        accumulate before {@code userPrompt} refuses to add more, forcing
 *        the caller to start a new chat
 */
